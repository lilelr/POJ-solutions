...
//        res += getSum(root * 2, left, right, L, mid);
        ...
        int l_res = getSum(root * 2, left, mid, L, mid);
        int r_res = getSum(root * 2 + 1, mid + 1, right, mid + 1, R);
        res = max(l_res,r_res);
    }
    return res;
}
